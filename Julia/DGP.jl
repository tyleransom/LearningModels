function DGP(N=10_000,T=10,J=8)
    NT=N*T
    # S=2
    ...
    sig_ans = round.(10*rand(J,1).+1;digits=0)./10 
    ...
    y = zeros(NT,J)
    for j=1:J
        y[:,j] = x[:,:,j]*b[:,j] + kron(A[:,j],ones(T,1)) + sqrt(sig_ans[j])*randn(NT,1)
    end
    ...
    return IDlp,time,N,T,S,y,x,Clp,A,b,sig_ans,Delta_ans,DeltaCorr_ans
end
